window.addEventListener('load', function () {
    ...
    if (!localStorage.getItem(TIP_KEY)) {
        ...
        mask.style = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:15px;';
        const box = document.createElement('div');
        box.style = 'max-width:520px;width:100%;background:#fff;border-radius:10px;padding:20px;max-height:80vh;overflow-y:auto;';
        box.innerHTML = `
...
4.本程序网络信号弱的时候会造成APP加载缓慢，属于正常现象，数据不会丢失！
            ...
            <div style="margin:16px 0;display:flex;gap:20px;justify-content:center;">
                <a href="隐私政策链接" target="_blank" style="color:#0066ff">隐私政策</a>
                <a href="服务协议链接" target="_blank" style="color:#0066ff">用户服务协议</a>
            </div>
            <button id="agreeBtn" style="width:100%;padding:10px;background:#2563eb;color:#fff;border:0;border-radius:6px;font-size:15px;">我已阅读并同意，开始使用</button>
        `;
